...
public class DetailScreenPageActions extends GetPage {
    ...
    public void selectLastAvailableHours(String string) {
        ...
        // String selectedTimeFormat = (String) executeJavascript("return
        // document.getElementsByClassName('rw-input')[0].textContent");
    ...
    private void verifyCurrentInformationAlerts(String appName) {
        try {
            // NOTE: Unable to click on 'Current Informational Alerts' button
            // due to
    ...
    public void verifyDropDownOptionsForLastHours() {
        ...
        String selectedTimeFormat = (String) executeJavascript(
                "return document.getElementsByClassName('rw-input')[0].textContent");
        System.out.println("Value of Time Format: " + selectedTimeFormat);
        if (!(selectedTimeFormat.equalsIgnoreCase(viewTimeFormat))) {
            executeJavascript("document.getElementsByClassName('rw-input')[0].click()");
            for (WebElement listItem : elements("list_timeViewFormat")) {
        ...
        selectedTimeFormat = (String) executeJavascript(
                "return document.getElementsByClassName('rw-input')[0].textContent");
        System.out.println("Value of Time Format: " + selectedTimeFormat);
        if (!(selectedTimeFormat.equalsIgnoreCase(viewTimeFormat))) {
            executeJavascript("document.getElementsByClassName('rw-input')[0].click()");
    ...
    public void verifyInformationAvailableForLastHours(String lastHours, String hours, String appName) {
        ...
        try {
            System.out.println("\n############## Verifying Information Available for " + lastHours
                    + " on Detail Screen Page, " + JiraId + " ##############\n");
            ...
            ReportMsg.info("App Information is not available, Message is appearing with text :- " + message + " for "
                    + lastHours);
            userNavigateToPlatfromAvailableScreenWhenClickOnPlatformAvailabilityHome();
            isElementDisplayed("singleApp", appName);
            element("singleApp", appName).click();
    ...
    public void verifyInformationAvailableForLast30Days(String last_30_days, String appName) {
        System.out.println("\n############## Verifying Information Available for " + last_30_days
                + " on Detail Screen Page, TRUST-323 ##############\n");
    ...
    public void checkForColorNotationGreenInThePlatformAvailabilityAndDetailScreen(String appName) {
        ...
        try {
            ...
            ReportMsg.info("Number of current time frames in green color are " + size
                    + " for 12 hours and EST time zone on Detail Screen");
            userNavigateToPlatfromAvailableScreenWhenClickOnPlatformAvailabilityHome();
        } catch (Exception e) {
            ReportMsg.info("table system status is not availabe for app");
            isElementDisplayed("div_errorMessage");
    ...
    public void checkForColorNotationInThePlatformAvailabilityAndDetailScreen(String appName, String color) {
        System.out.println("\n############## Verifying " + color
                + " color notation for 12 and 24 hours, TRUST-347 ##############\n");
        // // isElementDisplayed("div_appColorOnPlatform", appName);
        // ReportMsg.info("Verified color for " + appName + " app is green on
        // Platform Availability Screen");
        // isElementDisplayed("singleApp", appName);
        // element("singleApp", appName).click();
        // ReportMsg.info("Click on App = " + appName);
        isElementDisplayed("txt_appName");
        ...
        ReportMsg.info(
                "****** Verifying Slight disruption ,Minor disruption ,Service temporarily unavailable ,orange and Planned maintenance color for 12 hours ******");
    ...
    public void getTheColorStatusOfTimeFrame(String string) {
        verifyNumOfRowsAvailableOnForSelectedHours(string);

        for (WebElement ele : elements("list_housOnFrame")) {

            String hours = ele.getText();
            Iterator itr = timeList.iterator();
            ReportMsg.log("\n***** Chechking Color Status at '" + hours + "' hours ********\n");

            isElementDisplayed("div_cellStatus", hours);
            List<WebElement> eb = driver.findElements(By.xpath("//td[text()='" + hours + "']/ancestor::tr//td[contains(@class,'cell-status ')]//div"));
            ReportMsg.info("size " + eb.size());

            for (int i = 0; i < 12; i++) {
                WebElement el = eb.get(i);
                String minuteTime = itr.next().toString();
                String color = el.getAttribute("class");
                String time = hours + " " + minuteTime;
                ReportMsg.info("Color status at time '" + time + " is = " + color);
    ...
    private void verifyNumOfRowsAvailableOnForSelectedHours(String string) {
        isElementDisplayed("list_numOfRows");
        int rowSize = elements("list_numOfRows").size();
        ReportMsg.info("Num of rows for '" + string + "' is '" + rowSize + "' ");
    }

    public void verifyCellStatusOfCurrentTime(String string) {
        isElementDisplayed("time_CurrentHour");
        String currentHour = element("time_CurrentHour").getText();
    }
}
