...
public class CreateNotificationActions extends GetPage {
	...
	public void verifyMessageAndLogoIsDisplayingOrNotDisplayingToUnauthenticatedUser(String splashPageView,
		...
		try {
			...
			ReportMsg.info("Verified " + "'" + message + "'" + " is displaying to Unauthenticated user 'with logo' for "
					+ " '" + eventType + "' " + " event Type");
		} catch (NoSuchElementException e1) {
			try {
				isElementDisplayed("span_notification", message);
				ReportMsg.info(
						"Verified " + "'" + message + "'" + " is displaying to Unauthenticated user 'without logo' for "
								+ " '" + eventType + "' " + "event Type");
			} catch (NoSuchElementException e) {
				ReportMsg.info("Verified " + "'" + message + "'"
						+ " is not displaying to Unauthenticated user as notification is hided");
			}
		}
	}
	...
	public void verifyMessageAndLogoIsDisplayingOrNotToUnauthenticatedUserForDifferntColor(String splashPageView,
			String message, String eventType, String color) {
		if (splashPageView.contains("Grouping")) {
			ReportMsg.info(" Verifying created notification under Grouping View splash page");
		} else {
			ReportMsg.info("Verifying created notification under Front View splash page");
		...
		try {
			if (color.contains("red")) {
				...
				ReportMsg.info("Red color noitification is displaying with logo for " + "'" + eventType + "'"
						+ " event type with " + "'" + message + "'" + " message");
			}
			if (color.contains("green")) {
				isElementDisplayed("span_greenNotificationWithLogo", message);
				ReportMsg.info("Green color noitification is displaying with logo for " + "'" + eventType + "'"
						+ " event type with " + "'" + message + "'" + " message");

			}
			if (color.contains("orange")) {
				isElementDisplayed("span_orangeNotificationWithLogo", message);
				ReportMsg.info("Orange color noitification is displaying whith logo for " + "'" + eventType + "'"
						+ " event type with " + "'" + message + "'" + " message");
	...
	public void verifyOverriddenSystem(String monitorName) {
		...
		ReportMsg.info("Green color bricks are displaying under current time " + "'" + currentTime + "'"
				+ " on Detail Screen Pagge for " + monitorName);
	}
	...
	public void verifyOverriddenSystemForRegressionTest(String monitorName, String color) {
		try {
			...
			ReportMsg.info("'" + color + "' color bricks are displaying under current time " + "'" + currentTime + "'"
					+ " on Detail Screen Page for '" + monitorName + "' monitor ");
		} catch (NoSuchElementException e) {
			ReportMsg.info("Verified color notification is not displaying on detail screen page for '" + monitorName
					+ "' monitor");
		}
	}
